...
pub const CATALOG: &[OpDef] = &[
    ...
    OpDef {
        ...
        label: "Merge",
        cat: Category::FileOps,
        info: "Join files end to end. The streams are copied rather than re-encoded, so the files have to match: same codec, same size, same shape.",
        preview: Preview::DryRun,
        needs: &["ffmpeg"],
        fields: &[
            files(
                ...
                AV,
                true,
                "same codec, size and shape — the streams are copied, not re-encoded",
            ),
