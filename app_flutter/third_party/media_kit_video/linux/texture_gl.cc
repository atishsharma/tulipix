...
gboolean texture_gl_populate_texture(FlTextureGL* texture,
  ...
  if (required_width > 0 && required_height > 0) {
    ...
    int flip_y = 0;
    // Tulipix patch: never wait for the frame's display time here. This runs
    // on Flutter's raster thread, and mpv's default (1) blocks it for up to
    // `video-timing-offset` (50 ms) on every video frame -- the whole app,
    // video included, then draws at whatever rate that leaves, which is the
    // stutter a standalone mpv never shows (it waits on its own thread).
    // Flutter paces frames to vsync itself; mpv only has to hand one over.
